...
void gogo(char m[][1010],Queue &q,int boundr,int boundc,long long counter,long long count);
...
if(total==count){
    break;
            ...
            //cout<<total<<endl;
  gogo(Map,q,row,col,batlife/2,count);
         //when goes back we will have to go exactly the same
         id=0;
if(total==count){
    break;
}
...
while(1){
       ...
       BFS(m,goback,boundr,boundc,'0',counter);
       temp=goback.rear;
        }
